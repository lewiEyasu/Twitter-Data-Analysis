...
class TweetDfExtractor:
    ...
    def find_sentiments(self, text)->list:
        ...
        subjectivity = [TextBlob(text['full_text']).sentiment.subjectivity for text in self.tweets_list]
        polarity = [TextBlob(text['full_text']).sentiment.polarity for text in self.tweets_list]
    ...
    def find_hashtags(self)->list:
        hashtags_list = [tweet["entities"]["hashtags"] for tweet in self.tweets_list]
        hashtags = [] 
        for text in hashtags_list:
            if len(text) == 0 :
                hashtags.append("")
            else:
...
if __name__ == "__main__":
    ...
    _, tweet_list = read_json("/home/lewi/Downloads/Compressed/global_twitter_data.json")
